...
> [!NOTE]
> 自 2024/08/21 起，部分规则现在由 BTN 服务器直接生成更新，更新数据仅包含最近 14 天活跃数据。当超过 14 天 IP 不活动时，将从列表中自动移除。  
> 我们于 2024/08/20 日对 BTN 系统进行了升级，升级后需要一段时间积累数据。在此期间，规则数据量可能有所下降，但会随时间逐渐恢复。
